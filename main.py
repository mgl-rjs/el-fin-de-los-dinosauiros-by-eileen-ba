...
def velociraptor():
    ...
    raptor.start_effect(effects.trail)
def Nivel4():
    global triceratops
    game.show_long_text("Encuentra un lugar seguro con el triceratops",
        DialogLayout.CENTER)
    music.stop_all_sounds()
    triceratops = sprites.create(assets.image("""
        triceratops
    """), SpriteKind.player)
    controller.move_sprite(triceratops)
    tiles.set_tilemap(tilemap("""
        laberinto1
    """))
    tiles.place_on_random_tile(triceratops, sprites.castle.tile_dark_grass2)
    scene.camera_follow_sprite(triceratops)
...
def on_on_overlap2(sprite7, otherSprite4):
    if nivel == 3:
        if otherSprite4.right - sprite7.left < 2:
            info.change_score_by(1)
            music.power_up.play()
sprites.on_overlap(SpriteKind.player, SpriteKind.obstaculo, on_on_overlap2)
...
def borrar_sprites_3():
    for value8 in sprites.all_of_kind(SpriteKind.player):
        value8.destroy()
    for value23 in sprites.all_of_kind(SpriteKind.obstaculo):
        value23.destroy()
    for value33 in sprites.all_of_kind(SpriteKind.projectile):
        value33.destroy()
def piso3():
...
def on_on_overlap3(sprite3, otherSprite):
    if nivel == 3:
        scene.camera_shake(2, 500)
        info.change_life_by(-1)
        otherSprite.set_flag(SpriteFlag.AUTO_DESTROY, True)
...
def Nivel3():
    ...
    effects.blizzard.start_screen_effect()
    info.set_score(0)
    pterodactilo = sprites.create(assets.image("""
        pterodactilo
    """), SpriteKind.player)
    pterodactilo.ay = 300
def inicio():
...
def on_on_overlap4(sprite6, otherSprite3):
    ...
    if nivel == 1:
        ...
        nivel += 1
        Crear_Nivel()
sprites.on_overlap(SpriteKind.player, SpriteKind.goal, on_on_overlap4)
...
def Nivel5():
    game.show_long_text("Los dinosaurios no tuvieron salvación, el meteorito los extinguio...",
        DialogLayout.CENTER)
    music.stop_all_sounds()
    if info.life() > 0:
        if nivel == 5:
...
def on_on_overlap5(sprite24, otherSprite24):
    if nivel == 2:
        otherSprite24.destroy()
        scene.camera_shake(2, 500)
        music.power_down.play()
        info.change_life_by(-1)
sprites.on_overlap(SpriteKind.player, SpriteKind.roquitas, on_on_overlap5)

def on_on_overlap6(sprite23, otherSprite23):
    if nivel == 2:
...
def on_on_overlap7(sprite22, otherSprite22):
    ...
    if nivel == 1:
        ...
        scene.camera_shake(2, 500)
        info.change_life_by(-1)
sprites.on_overlap(SpriteKind.player, SpriteKind.rocones, on_on_overlap7)
...
def nubes_fuego():
    ...
    nubes_fuego2.x = 165
    nubes_fuego2.start_effect(effects.fire)
def Crear_Nivel():
    borrar_sprites_1()
    borrar_sprites_2()
    borrar_sprites_3()
    ...
    if nivel == 1:
        ...
        Nivel5()
def borrar_sprites_4():
    for value9 in sprites.all_of_kind(SpriteKind.player):
        value9.destroy()
    for value24 in sprites.all_of_kind(SpriteKind.background):
...
def on_update_interval7():
    if nivel == 2:
        rocas_fuego()
game.on_update_interval(1500, on_update_interval7)
